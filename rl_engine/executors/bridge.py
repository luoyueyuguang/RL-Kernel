...
class IPCWeightBridge:
    ...
    It leverages the CUDA IPC mechanism to achieve zero-copy weight synchronization, significantly reducing communication overhead.
       - Training process: Exports IPC handles for model weights.
       - Inference process: Reconstructs Tensors based on these handles, directly accessing the training process's memory.
    ...
    def export_model_handles(self, model: torch.nn.Module) -> Dict[str, Any]:
        ...
        Iterates through the model parameters and generates a cross-process readable IPC handle for each Tensor.
        """
    ...
    def import_model_weights(self, ipc_handles: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        Reconstructs Tensors based on the received handles, directly accessing the training process's memory.
        """
